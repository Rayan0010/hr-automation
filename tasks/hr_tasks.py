...
class HRTasks:
    ...
    def scrape_profiles(self, job_role):
        return Task(
            description=f"Scrape and analyze profiles matching the '{job_role}' position. Gather relevant information about candidates.",
            agent=self.profile_scraper_agent(job_role),
            expected_output="Collection of candidate profiles with relevant details."
        )

    def communicate(self):
        return Task(
            description="Contact shortlisted candidates to inform them about their selection and provide next steps.",
            agent=self.communication_agent(),
            expected_output="Communication sent to candidates with appropriate follow-up details."
        )
